...
class VideoTranslationUI:
    ...
    def translate_video(self, videoType, yt_link, video_path, tts_engine, language_eleven, language_edge, use_captions: bool, voice: str, progress=gr.Progress()) -> str:
        ...
        try:
            ...
            def logger(prog_str):
                progress(self.progress_counter / (num_steps), f"Translating your video - {prog_str}")
            content_translation_engine.set_logger(logger)

            for step_num, step_info in content_translation_engine.makeContent():
                progress(self.progress_counter / (num_steps), f"Translating your video - {step_info}")
            ...
            return self.embedHTML + '</div>', gr.Button.update(visible=True), gr.update(visible=False)
    ...
    def inspect_create_inputs(self, videoType, video_path, yt_link):
