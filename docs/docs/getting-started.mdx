...
facts_video_topic = "Random facts on dolphins"
content_engine = FactsShortEngine(voiceModule=voice_module,
    facts_type=facts_video_topic,
    background_video_name="minecraft background cube", # <--- use the same name you've saved in  the AssetDatabase
    background_music_name='chill music', # <--- use the same name you've saved in  the AssetDatabase
    num_images=0, # If you want images, put a number, preferrably below 25
